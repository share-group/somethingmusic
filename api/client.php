...
function uc_avatar($uid, $type = 'virtual', $returnhtml = 1) {
	...
	if($returnhtml) {
		return '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" width="480" height="480" id="mycamera" align="middle">
			...
			<embed src="'.$uc_avatarflash.'" quality="high" bgcolor="#eeeeee" width="480" height="480" name="mycamera" align="middle" allowScriptAccess="always" allowFullScreen="false" scale="exactfit"  wmode="transparent" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
		</object>';
	} else {
		return array(
			'width', '450',
...
function uc_get_avatar($uid, $size = 'middle', $type = '') {
	...
	$file = realpath(dirname(__FILE__).'../../').'/ucenter/data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).$typeadd."_avatar_$size.jpg";
	if(file_exists($file)){
		return UC_API.'/data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).$typeadd."_avatar_$size.jpg";
	}
	else {
