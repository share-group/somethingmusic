...
class control extends adminbase {
	...
	function onadd() {
		if(!$this->submitcheck()) {
			...
			$a = getgpc('a');
			$this->view->assign('a', $a);
			$typelist = array('UCHOME'=>'UCenter Home','XSPACE'=>'X-Space','DISCUZ'=>'Discuz!','SUPESITE'=>'SupeSite','SUPEV'=>'SupeV','ECSHOP'=>'ECShop','ECMALL'=>'ECMall','soft'=>$this->lang['soft'],'website'=>$this->lang['website'],'OTHER'=>$this->lang['other']);
	...
	function onping() {
		...
		if($app['extra']['apppath'] && @include $app['extra']['apppath'].'./api/'.$app['apifilename']) {
			...
			if($status == '1') {
				...
				echo 'document.getElementById(\'status_'.$appid.'\').innerHTML = "<img src=\'images/error.gif\' border=\'0\' class=\'statimg\' \/><span 
				class=\'red\'>'.$this->lang['app_connent_false'].'</span>";testlink();';
			}
	}
